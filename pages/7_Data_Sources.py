...
if st.button('COSTRUISCI DATASET MASSIMO',type='primary',use_container_width=True):
    try:
        ...
        master,report=build_dataset(
            ...
            bigballs_token=bigballs_token or None,use_public_team_context=use_team,
            use_fco_history=use_fco,use_fantacalcio_dev_history=use_dev,
            use_big_five_newcomer_history=use_big,
        )
        st.session_state.players=master
        st.session_state.coverage=report
...
if 'players' in st.session_state and isinstance(st.session_state.players,pd.DataFrame) and not st.session_state.players.empty:
    ...
    for c,(k,v) in zip(cols,metrics.items()): c.metric(k,v)
    ...
    if st.button('Aggancia anche prezzi medi aste reali pubbliche'):
        try:
            ...
            st.session_state.players=attach_market_prior(df,market,managers,budget)
            st.session_state.pop('scored',None)
            st.success(f'Prior aste reali agganciato ({len(market)} righe sorgente).')
            st.rerun()
        except Exception as e:
            st.warning(f'Fonte aste non disponibile: {e}')

    show=[c for c in ['player','team','role','quotation','fvm_1000','market_auction_price','minutes','xg','xa','dev_avg_vote','dev_fantamedia','external_minutes','external_xg','external_xa','team_attack_strength','team_defense_strength','data_confidence'] if c in df]
