...
sources=pd.DataFrame([
    ...
    ['OpenFootball Italy','Calendario 2026/27','380 partite Serie A, CC0; forza calendario combinata con Elo','CC0 / no key','SCHEDULE'],
...
sources.columns=['Fonte','Layer','Dati','Accesso','Uso nel motore']; st.dataframe(sources,use_container_width=True,hide_index=True)

st.info('Le chiavi possono essere incollate qui oppure impostate una volta nell’ambiente come FOOTBALL_DATA_TOKEN, API_FOOTBALL_TOKEN e BIGBALLS_TOKEN.')
st.subheader('Costruisci master arricchito')
a,b=st.columns(2)
with a:
    football_token=st.text_input('football-data.org token',value='',type='password',placeholder='usa env se vuoto')
    api_football_token=st.text_input('API-Football token (opzionale)',value='',type='password',placeholder='usa env se vuoto')
    bigballs_token=st.text_input('BigBalls token (opzionale)',value='',type='password',placeholder='usa env se vuoto')
    st.caption(f"Env: football-data={'✓' if os.getenv('FOOTBALL_DATA_TOKEN') else '—'} · API-Football={'✓' if os.getenv('API_FOOTBALL_TOKEN') else '—'} · BigBalls={'✓' if os.getenv('BIGBALLS_TOKEN') else '—'}")
    season=int(st.number_input('Anno iniziale stagione',2020,2030,2026)); label=st.text_input('Stagione Fantacalcio','2026/27')
with b:
    use_team=st.toggle('football-data.co.uk team context',True); use_elo=st.toggle('ClubElo current team strength',True); use_schedule=st.toggle('OpenFootball calendario + schedule strength',True); use_dev=st.toggle('fantacalcio.dev multi-season archive',True); use_fco=st.toggle('Fantacalcio-Online cross-check',True); use_api=st.toggle('API-Football stats + injuries',True); use_big=st.toggle('BigBalls big-five newcomers',True)
...
if st.button('COSTRUISCI DATASET MASSIMO',type='primary',use_container_width=True):
    try:
        ...
        master,report=build_dataset(season,label,football_token or None,fantasy_df=fdf,require_current_fanta=True,bigballs_token=bigballs_token or None,api_football_token=api_football_token or None,use_public_team_context=use_team,use_clubelo=use_elo,use_openfootball_schedule=use_schedule,use_fco_history=use_fco,use_fantacalcio_dev_history=use_dev,use_api_football=use_api,use_big_five_newcomer_history=use_big)
        st.session_state.players=master; st.session_state.coverage=report; st.session_state.pop('scored',None); st.success(f'{len(master)} giocatori caricati · {report.certification}')
...
if 'players' in st.session_state and isinstance(st.session_state.players,pd.DataFrame) and not st.session_state.players.empty:
    df=st.session_state.players; st.divider(); st.subheader('Copertura effettiva')
    metrics={'Giocatori':len(df),'Squadre':df.team.nunique() if 'team' in df else 0,'FVM/mercato':int(df.get('has_market_data',pd.Series(False,index=df.index)).fillna(False).sum()),'Storico Serie A':int(df.get('has_history',pd.Series(False,index=df.index)).fillna(False).sum()),'Storico estero':int(df.get('has_external_history',pd.Series(False,index=df.index)).fillna(False).sum()),'API-Football':int(df.get('has_api_football',pd.Series(False,index=df.index)).fillna(False).sum()),'Injury facts':int(df.get('has_current_injury_fact',pd.Series(False,index=df.index)).fillna(False).sum()),'Team match context':int(df.get('has_team_context',pd.Series(False,index=df.index)).fillna(False).sum()),'ClubElo':int(df.get('has_clubelo',pd.Series(False,index=df.index)).fillna(False).sum()),'Schedule':int(df.get('has_schedule_context',pd.Series(False,index=df.index)).fillna(False).sum()),'Storico fantasy':int(df.get('has_fantasy_history',pd.Series(False,index=df.index)).fillna(False).sum())}
    cols=st.columns(4)
    ...
    if st.button('Aggancia anche prezzi medi aste reali pubbliche'):
        try:
            market=load_real_auction_averages(); rules=st.session_state.get('rules'); managers=int(getattr(rules,'managers',8)); budget=int(getattr(rules,'budget',500)); st.session_state.players=attach_market_prior(df,market,managers,budget); st.session_state.pop('scored',None); st.success(f'Prior aste reali agganciato ({len(market)} righe sorgente).'); st.rerun()
        except Exception as e:st.warning(f'Fonte aste non disponibile: {e}')
    show=[c for c in ['player','team','role','quotation','fvm_1000','market_auction_price','minutes','xg','xa','dev_avg_vote','dev_fantamedia','af_minutes','af_rating','af_key_passes','currently_injured','injury_reason','external_minutes','external_xg','external_xa','team_attack_strength','team_defense_strength','team_elo','team_elo_factor','schedule_ease_factor','schedule_opponent_elo','data_confidence'] if c in df]
