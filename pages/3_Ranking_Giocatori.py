...
k=st.columns(5)
k[0].metric('Giocatori',len(ranked)); k[1].metric('P',int((ranked.role=='P').sum())); k[2].metric('D',int((ranked.role=='D').sum())); k[3].metric('C',int((ranked.role=='C').sum())); k[4].metric('A',int((ranked.role=='A').sum()))

list_tab, detail_tab, top_tab = st.tabs(['Classifica completa','Scheda giocatore','Top per ruolo'])
with list_tab:
    f1,f2,f3,f4=st.columns([1,1,1.25,2])
    ...
    query=f4.text_input('Cerca giocatore o squadra',placeholder='es. Inter, Lautaro…')
    view=ranked.copy()
...
with detail_tab:
    ...
    a=st.columns(6)
    a[0].metric('Score',f"{row.get('independent_score_v1',0):.1f}")
    a[1].metric('Punti attesi',f"{row.get('independent_points',0):.1f}")
    a[2].metric('VORP',f"{row.get('vorp',0):.1f}")
    a[3].metric('Fair price',f"{row.get('independent_fair_price',0):.0f}")
    a[4].metric('Affidabilità',f"{row.get('reliability',0):.0%}")
    a[5].metric('Minuti',f"{row.get('projected_minutes',0):.0f}")
    c1,c2=st.columns(2)
    with c1:
        ...
        st.caption(f"xG/90 modellato: {row.get('model_xg90',0):.2f} · xA/90: {row.get('model_xa90',0):.2f}")
    with c2:
