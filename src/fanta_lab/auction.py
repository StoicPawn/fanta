...
class AuctionState:
    def __init__(self,rules:LeagueRules,manager_names:list[str],my_manager:str):
        ...
        self._sim_cache={}
    def add_purchase(self,purchase:AuctionPurchase):
        self.purchases.append(purchase); self._sim_cache.clear()
    ...
    def fingerprint(self):
        parts=[str(self.rules.budget),str(self.rules.managers),str(self.rules.min_bid),self.my_manager]
        parts += [f'{p.manager}|{p.player}|{p.role}|{p.price}' for p in self.purchases]
    ...
    def manager_aggression(self,m,role:str|None=None):
        ...
        # shrink small samples toward neutral behaviour
        raw=float(np.median(ratios)); weight=min(1.0,len(ratios)/6)
        return float(np.clip((1-weight)*1.0+weight*raw,.60,2.20))
...
def simulated_clearing_price(player_row:pd.Series,pool:pd.DataFrame,state:AuctionState,n:int=900)->dict:
    ...
    demand=state.demand_pressure(role); tier=supply['tier_pressure']; anchor*=1+.10*demand+.09*max(0,tier-1)
    seed=int(hashlib.sha256((name+state.fingerprint()).encode()).hexdigest()[:8],16)
    rng=np.random.default_rng(seed); clears=[]; rivals=state.competing_managers(role)
    for _ in range(n):
        bids=[]
        for m in rivals:
            reserve=max(0,sum(state.slots_left(m).values())-1)*max(1,state.rules.min_bid)
            hard=max(0,state.remaining(m)-reserve)
            if hard<state.rules.min_bid: continue
            aggr=state.manager_aggression(m,role); shock=float(rng.lognormal(mean=-.5*.16**2,sigma=.16))
            bids.append(min(hard,anchor*aggr*shock))
        if bids:
            bids.sort(reverse=True)
            if len(bids)>=2: clears.append(min(bids[0],bids[1]+state.rules.min_bid))
            else: clears.append(min(bids[0],max(state.rules.min_bid,.55*anchor)))
        else: clears.append(float(state.rules.min_bid))
    a=np.asarray(clears,dtype=float)
    result={'p50':float(np.quantile(a,.50)),'p65':float(np.quantile(a,.65)),'p80':float(np.quantile(a,.80)),
...
def live_recommendation(player_row:pd.Series,pool:pd.DataFrame,state:AuctionState,risk_tolerance:float=.58)->dict:
    ...
    my_budget=state.remaining(state.my_manager); total_left=sum(left.values()); mandatory=max(0,total_left-1)*max(1,state.rules.min_bid); hard_cap=max(0,my_budget-mandatory)
    ...
    confidence=float(player_row.get('reliability',.5) or .5); edge=float(player_row.get('edge_confidence_adjusted',player_row.get('edge_vs_market',0)) or 0)
    ...
    liquidity_factor=float(np.clip(state.room_liquidity()/max(1,role_budget_share),.85,1.18)); strategic*=liquidity_factor
    max_bid=int(min(hard_cap,max(state.rules.min_bid,math.ceil(strategic)))) if hard_cap>=state.rules.min_bid else 0
