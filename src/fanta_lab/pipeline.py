...
def _merge_team_source(master:pd.DataFrame, source:pd.DataFrame, threshold:int=82)->pd.DataFrame:
    ...
    src=source.rename(columns={'team':'player'})
    joined,_=fuzzy_join(teams,src,'_teamsrc',threshold); joined=joined.rename(columns={'player':'team'})
...
def build_dataset(season_start_year:int, fanta_season_label:str, football_token:str|None=None,
    ...
    if hist:
        agg=_weighted_player_history(hist,[.58,.27,.15]); master,unmatched_stats=fuzzy_join(master,agg,'_stats',89)
    ...
    # 3) Previous-season team environment from football-data.co.uk.
    if use_public_team_context:
        try:
            tf=FootballDataUKSource().team_features(season_start_year-1,'I1')
            if len(tf):
                master=_merge_team_source(master,tf,82)
                report.notes.append(f'football-data.co.uk: previous-season team context loaded for {int(master.team_attack_strength.notna().sum()) if "team_attack_strength" in master else 0} player rows.')
        except Exception as e:
            report.notes.append(f'football-data.co.uk unavailable: {type(e).__name__}')

    # 4) Current independent club strength from ClubElo, including promoted/relegated context.
    if use_clubelo:
        try:
            elo=ClubEloSource().italy(levels=(1,2))
            if len(elo):
                master=_merge_team_source(master,elo,80)
                report.notes.append(f'ClubElo: current Elo attached to {int(master.team_elo.notna().sum()) if "team_elo" in master else 0} player rows.')
        except Exception as e:
            report.notes.append(f'ClubElo unavailable: {type(e).__name__}')

    # 5) Second fantasy-history source.
    if use_fco_history:
        try:
    ...
    # 6) API-Football: detailed previous-season stats + current injury/suspension feed.
        ...
        try:
            ...
            if len(prev_players):
                ...
                if 'af_appearances' in master and 'af_lineups' in master:
                    apps=pd.to_numeric(master.af_appearances,errors='coerce'); starts=pd.to_numeric(master.af_lineups,errors='coerce')
                    master['starting_probability']=(starts/apps.replace(0,pd.NA)).clip(0,1)
                    master['starting_probability_source']='api-football previous-season lineup share'
                if 'af_penalties_missed' in master: master['penalties_missed']=pd.to_numeric(master['af_penalties_missed'],errors='coerce')
            try:
                current=af.serie_a(season_start_year); cov=current.get('coverage',{}); injuries_enabled=bool(cov.get('injuries',False)) if isinstance(cov,dict) else False
                if injuries_enabled:
                ...
                else: report.notes.append('API-Football current injury coverage flag is false/not started; no injury facts injected.')
    ...
    # 7) Big-five historical xG, especially for arrivals from abroad.
    if bigballs_token and use_big_five_newcomer_history:
        try:
            ...
            if len(bbh):
                ...
                nums=[c for c in ['matches','minutes','goals','assists','shots','key_passes','xg','xa','npxg','xg_chain','xg_buildup'] if c in bbh]; rows=[]
    ...
    master['has_clubelo']=master.get('team_elo',pd.Series(index=master.index,dtype=float)).notna()
    ...
    master['data_confidence']=(.16+.21*master['has_market_data'].astype(float)+.17*master['has_history'].astype(float)
        +.10*master['has_external_history'].astype(float)+.08*master['has_team_context'].astype(float)+.05*master['has_clubelo'].astype(float)
        +.10*master['has_fantasy_history'].astype(float)+.10*master['has_api_football'].astype(float)+.03*master['has_current_injury_fact'].astype(float)).clip(0,1)
