...
SOURCES=(
 ...
 SourceSpec('StatsBomb Open Data','historical-event-training','open-data',30*24*3600,60,False,('statsbomb_training_features',)),
...
def registry_frame():
    rows=[]
    for s in SOURCES:
        d=asdict(s); d['ttl_hours']=round(s.ttl_seconds/3600,1); d['fields']=', '.join(s.fields); rows.append(d)
    return pd.DataFrame(rows).sort_values(['critical','priority'],ascending=[False,False])

def source_for_field(field): return sorted([s for s in SOURCES if field in s.fields],key=lambda s:s.priority,reverse=True)

def refresh_plan(df=None):
    rows=[]
    for s in SOURCES:
        if df is None or df.empty: coverage=0.0
        else:
            present=[]
            for f in s.fields:
                if f not in df: present.append(pd.Series(False,index=df.index))
                else:
                    x=df[f]; present.append(pd.to_numeric(x,errors='coerce').notna() if pd.api.types.is_numeric_dtype(x) else x.notna())
            ok=present[0].copy()
